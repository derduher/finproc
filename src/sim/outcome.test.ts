...
describe('deriveOutcomeReads', () => {
  it('flags an over-saver: sustainable above target → under-spending', () => {
    const o = deriveOutcomeReads({ result: makeResult(), sustainable: 103_000, target: 80_000, maxAge: 95 })
  ...
  it('flags an under-funded plan: target above sustainable → over-spending', () => {
    const o = deriveOutcomeReads({ result: makeResult(), sustainable: 60_000, target: 80_000, maxAge: 95 })
  ...
  it('carries the surplus/legacy distribution and hold rate', () => {
    const o = deriveOutcomeReads({ result: makeResult({ successRate: 0.97 }), sustainable: 103_000, target: 80_000, maxAge: 95 })
