...
| Simulation | `src/sim/` | `account.ts` (SimAccount class) → `projection.ts` (single run) → `montecarlo.ts` (1,000 runs) → `sensitivity.ts` (OAT ±20%) → `cashflow.ts` (aggregated series) → `insights.ts` (rule-based cards) → `fundingCurve.ts` (required balance per age vs projected accumulation) |
...
| Hooks | `src/hooks/` | `useSimulation` (cache-first, 350ms debounced recompute — stale flag flips immediately, exposes `progress`), solver hooks (`useSustainableSpend`, `useEarliestRetirementAge`, `useRequiredExtraSavings`), `useSensitivity` / `useInsights` (tornado + cards), `useFundingCurve` (funding curve, cache-first + progress), `useHistoricalStress`, `useUrlSync` (500ms debounce), `useScenarios` (IDB, max 4) |
...
| UI | `src/ui/` | `v2/` single live screen (GuidedFirstRun → MainScreen + AdvancedDrawer/MethodologyDrawer), `charts/` raw-SVG charts (PathsChart, FundingChart, HiTornado, GuardrailTimeline, LegacyBar), `frame/` (Logo, UrlParseFailedBanner), `shared/` (Field, MoneyInput) |
...
- **Funding curve** — `src/sim/fundingCurve.ts` answers "how much do I need at each age, and what will I have?" Two series over `currentAge … min(maxAge − 1, MAX_SOLVE_AGE=80)`, both in **today's dollars**:
  - *Required*: for each age X, bisect the minimum starting balance that survives **each individual sampled path**, then read the confidence as a quantile of those per-run minima. This is an exact reformulation, not an approximation — run *i* survives balance B iff B ≥ its own minimum — so one solve serves every confidence level (the slider re-reads rather than re-solving) and the confidence dimension carries no MC noise. It relies on survival being monotone in the starting balance; a change that breaks that monotonicity silently corrupts every number here.
  - *Projected*: one never-retire accumulation pass (`retirementAge` pinned to `maxAge`, `longevity` forced to `fixed`) gives p10/p50/p90 per age, deflated by each run's **own realized price level** rather than the expected deflator `deflateResult` uses. The same pass supplies the median account mix, which becomes the template the requirement is solved against — so "$2M at 60" is asked of the tax split you're actually on track to hold.
  - The requirement is forced non-increasing in age (`R[age] ≥ R[age+1]`, swept right to left, at each *rank* so every quantile stays consistent and the per-age arrays stay sorted). Any rise is sampling noise: funding fewer remaining years can't need more money.
  - **The chart's crossing is not a retirement date.** It pits a median projection against a confidence-matched requirement, which factorizes a joint probability and reads optimistic. `FundingChart` draws the p10–p90 band so the crossing reads as a range and marks `findRetirementAgeForSuccess`'s age separately; `fundingNarrative.ts` says so in words when they disagree. Don't "simplify" this by deleting the marker.
  - Reference marks: the plan's own retirement age is labelled on the axis (`retire · 62`) rather than left as a bare tick, and `fullyFundedAge` marks the first age needing **no portfolio at all** — the one case where the requirement genuinely reaches zero, which takes guaranteed income meeting or exceeding spending. `fundingHorizonNote` states the solved range against the plan's end, since the curve stops at 80 while a plan may run to 100; under stochastic longevity it says the lifespan varies instead of quoting `maxAge`, which that mode barely uses.
  - Units: the requirement is solved by starting a projection *at* age X, where the price level restarts at 1, so it is natively real. There is no nominal toggle on this chart — converting would mean multiplying by a stochastic price path.
- **Early access (`withRetirementAge(inputs, age, { earlyAccess: true })`)** — cascades *every* account's `withdrawalStartAge` down to the retirement age (`min`, so it only ever unlocks), modelling Rule of 55 / 72(t) / a Roth ladder. Solvers (`retirementSolver`, `fundingCurve`) pass it; the **store's** retirement-age cascade does not, because that path edits the user's real account settings. Without it, a plan whose money is locked until 59½ "succeeds" at an early retirement age without funding the intervening years — `runSingleProjection` treats a withdrawal lockout as a *silent shortfall*, not depletion, so those unfunded years cost the run nothing.
...
| No funding-curve view | `FundingRead` + `FundingChart`: required-balance-per-age vs projected accumulation, with the solver's earliest age marked separately |
