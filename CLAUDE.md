...
- **Reliable-median horizon** — the per-year `p50`/`p10`/`p90` are computed over the runs that reach each year, with depleted runs zero-filled, so the median misbehaves at the extreme tail in two ways: under **fixed** longevity it snaps discontinuously to $0 once depleted runs cross half the cohort, and under **stochastic** longevity it goes wild when only a handful of old survivors remain (a "spike to $100M then crash"). `runMonteCarlo` trims the trailing `yearlyResults` to `reliableMedianYears(balancesByYear)` — the last age where **both** ≥`MEDIAN_DISPLAY_MIN_SOLVENT` (50%) of the runs reaching it are solvent **and** ≥`MEDIAN_DISPLAY_MIN_COHORT` (10%) of the *original* cohort still reach it. Success/`endBalances` stats stay over the full horizon; only the drawn median/band horizon is trimmed. Sample paths still extend to each run's end, so the spaghetti shows the long-lived tail and late depletions even though the median line stops. `PathsChart.summarizeColumn` returns an `undefined` median past the trim so the hover readout doesn't re-print a misleading value out there.
